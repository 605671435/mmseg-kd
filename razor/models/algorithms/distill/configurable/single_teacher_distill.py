...
from razor.utils.build_model import build_model
    ...
    def __init__(self,
        ...
        if isinstance(teacher, Dict):
            if isinstance(teacher['cfg_path'], str):
                ...
                if hasattr(teacher_cfg.model.backbone, 'drop_path_rate'):
                    teacher_cfg.model.backbone.drop_path_rate = 0.
                import seg.utils
                seg.utils.register_all_modules()
                # cfg_path = architecture['cfg_path'].split('::')[1]
                # teacher = SEG_MODELS.build(teacher_cfg.model)
                teacher = build_model(teacher_cfg.model, task)
            # teacher = MODELS.build(teacher)
            else:
                teacher = teacher['cfg_path']
                # teacher = SEG_MODELS.build(teacher)
                teacher = build_model(teacher, task)
                if hasattr(teacher.backbone, 'drop_path_rate'):
                    teacher.backbone.drop_path_rate = 0.
